...
ans_key = ['','C','D','A','B','B','B','C','B','C','A','D','A','B','C']
options = ['A','B','C','D']
money = [0,1000,3000,5000,10000,20000,50000,70000,90000,120000,
         300000,500000,750000,1000000,500000]
...
def life_line(index):
    ...
    if len(life_line_list) == 0:
        print("You loss your all lifelines")
    else:
        while counter == 0:
            ...
            if '50-50' in player_choice:
                ...
                print("You chooses " + player_choice + "\n")
                print("Wait for audience reply.....\n")
                time.sleep(3)
                highest = ans_key[index] + " : 65%"
...
def check_answer(qes_count,player_choice):
    ...
    elif qes_count == 11 and player_choice == ans_key[qes_count]:
        return("You Won " + str(money[qes_count]))
    elif qes_count == 12 and player_choice == ans_key[qes_count]:
        return("You Won " + str(money[qes_count]))
    elif qes_count == 13 and player_choice == ans_key[qes_count]:
        return("You Won " + str(money[qes_count]))
    elif qes_count == 14 and player_choice == ans_key[qes_count]:
        return("You Won " + str(money[qes_count]))
    elif qes_count == 15 and player_choice == ans_key[qes_count]:
        return("You Won 1 Crore rupees")

def kbc():
    qes_count = 0
    counter = 0
    instructions = '''......Welcome to KBC......\n\n1.You have four options for each question.\n2.You have Life Line also to use it type(life line.\n3.To Quit the game type (quit).\n'''
    print(instructions)
    with open('questions.json') as content:
        ...
        while counter < len(questions['data']):
            ...
            if 'QUIT' in player_choice:
                print("Thanks for playing nicely!! You Won "+str(money[counter]) + " Rupees ")
                break
            ...
            if 'A' in player_choice or 'B' in player_choice or 'C' in player_choice or 'D' in player_choice:
                ...
                if ans:
                    print(ans + "\n")
                     ...
                     print("Thanks for Playing Nicely!You Won "+str(money[qes_count-1]) + " Rupees")
                     break
                counter +=1
                if counter <=9:
                    ...
                    os.system('clear')
                    print("Enter valid options! like A or B or C or D:)\n")
                    counert = counter
